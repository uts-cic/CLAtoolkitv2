...
from .utils.date_utils import convert_to_datetime_object
...
class GithubImporter(object):
    ...
    xapi_verbs = [xapi_settings.VERB_CREATED, xapi_settings.VERB_ADDED, xapi_settings.VERB_REMOVED,
                  xapi_settings.VERB_UPDATED, xapi_settings.VERB_COMMENTED, xapi_settings.VERB_CLOSED,
                  xapi_settings.VERB_OPENED]
    ...
    def __init__(self, msg):
        self.token = msg['oauth_token'] # Probably not needed
        self.candidate_repos = msg['retreival_param'] # Array of repos/user tokens
        self.unit = msg['unit']

    def perform_import(self):
        for target_repo in self.candidate_repos:
            repo_url = self.platform_url + target_repo['identifier']
            print "GitHub data import target repository: " + repo_url

            gh = Github(login_or_token=target_repo['user_token'], per_page=self.per_page)
            repo = gh.get_repo(target_repo['identifier'])
            self.import_activities(self.unit, repo_url, target_repo['identifier'], repo, self.token)
            self.import_commits_from_all_branches(self.unit, repo_url, target_repo['identifier'], repo)
    ...
    def import_activities(self, unit, repo_url, repo_name, repo, token):
        ...
        self.import_issue_activities(unit, issue_number_list, repo_url, repo_name, repo, token)

    def import_issue_activities(self, unit, issue_list, repo_url, repo_name, repo, token):
        ...
        for issue in issue_list:
            ...
            for issue_event in issue_events:
                ...
                # TODO: Rework xapi statements for the case where only one user in toolkit
                # TODO: Assigner in toolkit but not assignee: Assigner assigned x to gh_user_id
                # TODO: Assignee in toolkit but not assigner: Assignee was assigned x by gh_user_id
                assigner_user = user_exists_in_toolkit(assigner_id, unit, self.platform)
                assignee_user = user_exists_in_toolkit(assignee_id, unit, self.platform)
                if assigner_user is not None and assignee_user is not None:
                    user = assigner_user
                    insert_added_object(user, issue_url, event_url, assignee_name, date, unit, self.platform,
                                        self.platform_url,
    ...
    def import_commit_comments(self, event, unit, repo_url, repo_name, repo):
        ...
        user = user_exists_in_toolkit(author_id, unit, self.platform)
    ...
    def import_commits(self, commit, unit, repo_url, repo_name):
        ...
        if user_exists_in_toolkit(author_id, unit, self.platform) is None:
    ...
    def get_detail_values_by_fetch_results(self, xapi_statements):
        ...
        for stmt in xapi_statements:
            ...
            # Date
            dt = convert_to_datetime_object(stmt['timestamp'])
    ...
    def get_object_values_from_context(self, stmt):
        ...
        if action == self.EVENT_TYPE_COMMIT:
            ...
            #     verb = 'Closed'
            value = "Opened %s in %s" % (
            self.italicize(object_val), self.italicize(contexts[1]['definition']['name']['en-US']))
